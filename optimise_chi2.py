#Folder results_chi/{system} must exist
filecsv = open(f"results_chi2/{system}/a{a_max}_p{p_miss}_alpha{significance_level}_w{window_length}.csv","a+")
filetxt = open(f"results_chi2/{system}/a{a_max}_p{p_miss}_alpha{significance_level}_w{window_length}.txt","a+")
filecsv.seek(0)
...
for k in range(a_max+1):
    #Counting the number of k-consecutive misses
...
for k in range(a_max+1):
    ...
    #observed
    obs = ms[k]
    #Remove division with N and move to bound instead
